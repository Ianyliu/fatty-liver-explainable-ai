...
def residuals(clf, X, y, r_type='standardized'):
    ...
    y_true = y.astype(np.float32).copy().squeeze()
...
def summary(clf, X, y, xlabels=None):
    ...
    # ll_model = np.sum(y * np.log(y_pred_proba) + (1 - y) * np.log(1 - y_pred_proba))
    # ll_null = np.sum(y * np.log(np.mean(y)) + (1 - y) * np.log(1 - np.mean(y)))
    # pseudo_r_squared = 1 - (ll_model / ll_null)
    
    # # Calculate pseudo R-squared (based on MSE reduction)
    # y_pred = clf.predict(X)
    # mse_model = np.mean((y - y_pred)**2)
    # mse_null = np.mean((y - np.mean(y))**2)
    # pseudo_r_squared = 1 - (mse_model / mse_null)
    ...
    #     (np.round(np.array([clf.intercept_]), 6), np.round(clf.coef_, 6).squeeze()))
    coef_df['Estimate'] = np.concatenate(
        (np.round(clf.intercept_, 6), np.round(clf.coef_, 6).squeeze()))
    coef_df['Std. Error'] = np.round(coef_se(clf, X, y), 6)
    coef_df['95% Upper CI'] = coef_df['Estimate'] + coef_df['Std. Error']
    coef_df['95% Lower CI'] = coef_df['Estimate'] - coef_df['Std. Error']
    ...
    # print(f"Pseudo-R-square (based on MSE reduction): {pseudo_r_squared}")